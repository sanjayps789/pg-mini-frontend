...
export default function FreelancerDashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (projectRes.status === "fulfilled") {
          ...
          console.warn("Projects fetch failed:", projectRes.reason);
        }

        if (proposalRes.status === "fulfilled") {
          setProposals(proposalRes.value.data || []);
        } else {
          console.warn("Proposals fetch failed:", proposalRes.reason);
        }

        if (notificationRes.status === "fulfilled") {
          setNotifications(notificationRes.value.data || []);
        } else {
          console.warn("Notifications fetch failed:", notificationRes.reason);
        }

        if (balanceRes.status === "fulfilled") {
          setEarnings(balanceRes.value.data?.balance || 0);
        } else if (balanceRes.reason?.response?.status === 404) {
          console.log(
            "Freelancer bank account not set up yet, earnings default to 0."
          );
        } else {
          console.warn("Balance fetch failed:", balanceRes.reason);
        }
      } catch (error) {
        toast.error("Unexpected error occurred while fetching dashboard data.");
  ...
  //           pr.status === "accepted" &&
  //           pr.project &&
  //           pr.project.status !== "completed"
  //       )
  //       .map((pr) =>
  //         projects.find(
  //           (p) => String(p._id) === String(pr.project?._id || pr.project)
  //         )
  //       )
  //       .filter(Boolean),
  //   [proposals, projects]
  ...
  const handleFectActiveProjects = async () => {
    try {
        const res = projects.filter((p) => p.status === "in-progress" || p.status === "completed" );
  ...
  console.log("Our Active Projects" ,activeProjects);


  useEffect(() => {
    handleFectActiveProjects();
  }, [projects]);
          ...
          <div className="space-y-4 flex-1">
            {openProjects.length > 0 ? (
                ...
                >
                  ...
                  <div className="text-sm text-gray-500 mt-2">
                    ...
                    <p className="flex items-center">
                      Rating: {project.client?.rating || 0}/5{" "}
                  ...
                  <div className="mt-4 pt-4 border-t flex items-center justify-between">
                    ...
                    {(() => {
                      const myProposal = proposals.find(
                        (pr) =>
                          String(
                            pr.project && (pr.project._id || pr.project)
                          ) === String(project._id)
                      );
                      if (myProposal) {
                        if (myProposal.status === "pending") {
                          return (
                            <div className="flex flex-col sm:flex-row gap-2">
                              ...
                              <button
                                onClick={() =>
                                  handleWithdrawProposal(myProposal._id)
                                }
                        ...
                        if (myProposal.status === "accepted") {
                          return (
                            <button
                              disabled
                              className="w-full bg-green-100 text-green-700 py-2 rounded-lg font-medium"
                            >
                              Accepted ✅
                            </button>
                          );
                        ...
                        if (myProposal.status === "rejected") {
                          return (
                            <button
                              disabled
                              className="w-full bg-red-100 text-red-700 py-2 rounded-lg font-medium"
                            >
                              Rejected ❌
                            </button>
                          );
                        ...
                        return (
                          <button
                            disabled
                            className="w-full bg-gray-300 text-gray-700 py-2 rounded-lg font-medium"
                          >
                            {myProposal.status}
                          </button>
                        );
              ...
              <p className="text-gray-500">
                No open projects matching your search.
              </p>
            )}
          </div>
        </div>
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <aside
        className={`fixed lg:static top-0 left-0 h-full w-64 bg-white transform ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        } lg:translate-x-0 transition-transform duration-300 z-50 flex flex-col border-r`}
      >
        ...
        <nav className="flex-grow p-4 space-y-2 overflow-y-auto">
          ...
          >
            <FaTasks className="mr-3 h-5 w-5" /> Task Progress
          </NavLink>
          <NavLink
            to="/messages"
            className={({ isActive }) =>
          ...
          >
            <FaStar className="mr-3 h-5 w-5" /> Rate a Client
          </NavLink>
          <NavLink
            to="/reviews"
            className={({ isActive }) =>
          ...
          >
            <FaStar className="mr-3 h-5 w-5" /> My Reviews
          </NavLink>
          <NavLink
        ...
        <div className="p-2 border-t">
          <button
            ...
            className="w-full flex items-center px-4 py-2 text-gray-600 hover:bg-red-50 hover:text-red-600 rounded-lg"
          >
        ...
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-2">
              Find new projects and manage your work.
            </p>
          </div>
          <div className="flex items-center space-x-4 w-full sm:w-auto">
            ...
            <button
              className="lg:hidden p-2 bg-indigo-100 rounded-lg"
            ...
            <Link
              to="/notifications"
              className="hidden sm:flex items-center text-gray-600 hover:text-gray-800 relative"
            >
              <FaBell />
